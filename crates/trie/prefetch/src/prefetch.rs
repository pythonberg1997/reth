...
impl TriePrefetch {
                ...
                state = prefetch_rx.recv() => {
                    if let Some(state) = state {
                        ...
                        join_set.spawn(async move {
                            if let Err(e) = self_clone.prefetch_accounts::<DB>(consistent_view, hashed_state_clone, arc_tracker_clone).await {
                        ...
                        let consistent_view = ConsistentDbView::new_with_latest_tip(provider_factory.clone()).unwrap();
                        join_set.spawn(async move {
                            if let Err(e) = self_clone.prefetch_storages::<DB>(consistent_view, hashed_state).await {
    ...
    pub async fn prefetch_accounts<DB>(
        self: Arc<Self>,
    ...
    pub async fn prefetch_storages<DB>(
        self: Arc<Self>,
        consistent_view: ConsistentDbView<DB, ProviderFactory<DB>>,
        hashed_state: HashedPostState,
    ) -> Result<(), TriePrefetchError>
